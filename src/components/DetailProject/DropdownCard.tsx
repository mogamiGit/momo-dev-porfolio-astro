...
const DropdownCard: React.FC<Props> = ({ title, subtitle, emoji, className }) => {
  ...
  return (
    ...
    >
      <motion.div className="w-full group flex gap-3 items-center" layout="position">
        <motion.p
          className="text-xl"
          layout
        ...
        >{emoji}</motion.p>
        <motion.p className="w-full text-lg font-semibold line-clamp-2">{title}</motion.p>
